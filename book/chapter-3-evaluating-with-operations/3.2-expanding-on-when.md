...
{% hint style="warning" %}
The order of contexts in an operable does not determine what operations can qualify. In the given example, the same operation will be performed even if the contexts were reversed as `evaluate <validated response is Validated Int String, response as Unvalidated Int String>`. This provides flexibility in how operations define themselves, without binding evaluations to that definition.
