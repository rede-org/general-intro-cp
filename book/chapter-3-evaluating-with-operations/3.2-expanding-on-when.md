...
Notify user of bad input for Validated Int String: <input> 
    when not input (Is Valid Int)?
        await "\(input) is not a number." as Console Message.
```

{% hint style="danger" %}
Notice how functionality was added only by adding this new operation. In the traditional implementation of most other paradigms, this isn't the case. It is usually not sufficient to add only the new functionality, but also change any related functionality that will use it. Contextual Programming decouples functionality from other functionality through the use of `await` and indirect evaluation of the operations that qualify for the specified contexts.
...
Check whether evenr: when initialized?
    await "Please enter a number." as Console Message,
    await Console Response.
...
Check whether even: when initialized?
    await "Please enter a number." as Console Message,
    await response: Console Response;
    await response as Potential Number Response.
```
...
Notify user of Invalid Number Response: <response>?
        await "\(response) is not a number." as Console Message.
```
...
This last update changed `await response as Potential Number Response` to `await response as Potential Number Response or response as Invalid Number Response`. The noteworthy part of this change is the `or`, which defines the context(s) that follow as conditional await(s) to be performed. It basically says "if no operation occurs for `response as Potential Number Response`, then await `response as Invalid Number Response`.
...
{% hint style="info" %}
The opposite conditional await can be achieved with `and`, in which the following context(s) will only be evaluated by the await if the preceding evaluation had successfully performed at least one operation.
{% endhint %}

The use of conditional awaits allows chaining of awaits in a logical fashion that furthers the concept of "when" logic occurs.
