...
{% hint style="danger" %}
Notice how functionality was added only by adding this new operation. In the traditional implementation of most other paradigms, this isn't the case. It is usually not sufficient to add only the new functionality, but also change any related functionality that will use it. Contextual Programming decouples functionality from other functionality through the use of `evaluate` which indirectly executes the operations that qualify for the specified contexts.
