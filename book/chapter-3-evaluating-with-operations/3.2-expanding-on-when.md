...
{% hint style="info" %}
The angle brackets `<  >` around contexts actually builds an instance of a built-in type, in Rede, called an "operable". These can be evaluated to initiate operations that use all of the contexts contained within the operable and are reviewed in more detail in [a later chapter](broken-reference).
{% endhint %}
