...
Behaviors add quite a bit of complexity to the concept of 'When'. It's important to understand how the different features of behaviors define the flow of the application. This flow can be thought of as a cycle that loops until the app is terminated. The various aspects of the cycle can be listed as:
...
- [A] Execute qualified operation(s).
    ...
    -> On delayed activation (\activate ...).
        -> Schedule the activation of the context.
    -> On delayed deactivation (\deactivate ...).
        -> Schedule the deactivation of the context.
            -> De-schedule any pending activation of the same context.
    -> On continuation (continue).
        -> Schedule the operation to continue from this point.
    ...
    -> On delayed evaluation (\evaluate ...).
        -> Schedule the evaluation of the specified contexts.
- After all qualified operations complete.
    -> [D] Perform scheduled evaluations.
    -> [E] Perform scheduled activations/deactivations.
        -> Repeat [B] for each scheduled activation.
            -> Resume here once all activations complete.
        -> Repeat [C] for each scheduled deactivation.
            -> Resume here once all deactivations complete.
    -> [F] Repeat [A] for all (behavior) operations that reactively qualify for 
        the current cycle's changes.
    -> Resume any operations that were scheduled through a continuation.
```

There's a lot going on in each cycle. The following subsections break it down into parts that are more concise to what usually matters to a developer.
...
Delayed activation/deactivation merely schedules activation/deactivation to occur after all currently qualifying and otherwise scheduled operations have completed. An activation that has been scheduled will be de-scheduled if it is deactivated prior to those scheduled activations being performed. All scheduled activations are effectively performed [together](4.2-from-when-to-whenever.md#activating-together).

### Continuation

A continuation, specified through code as `continue` and briefly introduced [earlier](4.1-revisiting-hello-world.md#deactivating), pauses the execution of the current operation and runs through the remainder of the current cycle before continuing it. This is most often used to let reactive evaluations complete mid-operation, so the remainder of the operation can assume its earlier changes have propagated or have been validated by other operations.

There is no delayed continuation with `\continue` as that would have no effect.
