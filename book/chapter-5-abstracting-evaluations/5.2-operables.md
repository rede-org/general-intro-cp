...
{% hint style="info" %}
While operables are similar to compositions in many ways, one difference is that they cannot be held as a value within a context. This is due to contexts only persisting in memory if they are activated and bound to at least one behavior. With compositions, it doesn't really matter if a context is destroyed as it simply means that context won't be considered for compositional evaluations (any behaviors that would have expected it already don't exist).&#x20;

However, for operables, it could be confusing as to why an operable that explicitly states that it has a context won't actually evaluate an operation that expects that context, simply because that context falls out of scope or was deactivated somewhere. To prevent this potential confusion, Rede has opted to only permit operables to persist within a chain of operations or as a qualification value.
{% endhint %}
        ...
        foreach i in Range[1, fizz buzz]?
{
    ...
    Otherwise :: 
        default?
            on default (0) is Neither Fizz Nor Buzz Number,
            evaluate on default;
}.
```
