...
Just like other types, composition types can be useful to restrict what kind of compositions an operation may accept. They can also be used with the composition fulfillment operator `>#`. This operator will attempt to expand a composition until it fulfills a specific composition type. If it can't, then it returns back the closest it [could get](#user-content-fn-7)[^7], minus any contexts that are not required to fulfill a composition type are removed from the returned composition.
...
Given the `target` composition, indexing the first element with `target(0)` will provide the composition equivalent to `{target id}`. Even indexing that, `{target id}(0)`, will provide the composition equivalent to `{target id}`. This is due to how compositions reference contexts but can be maintained as values within contexts. If the contexts of the composition could be retrieved, the questions originally raised about nested contexts arise [again](../chapter-2-creating-context/2.1-organizing-data.md#contexts-vs.-records).
...
{% hint style="info" %}
Operations with input compositions may use the general `Composition` type or a specific composition type, although the latter is much more restrictive.
{% endhint %}
